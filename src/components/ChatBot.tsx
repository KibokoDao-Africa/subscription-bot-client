...
export const ChatBot = () => {
  ...
  const toggleVoiceCommand = () => {
    if (isListening) {
      ...
      setIsListening(true);
      voiceCommands.startListening(handleVoiceCommand);
    }
  };
  const handleVoiceCommand = (command: string) => {
    switch (command) {
      case "createChannel":
        handleChannelAction("create");
        voiceCommands.speak(
          "Creating a new channel. Please provide the channel name.",
        );
        break;
      case "deleteChannel":
        handleChannelAction("list");
        voiceCommands.speak("Select a channel to delete.");
        break;
      case "subscribe":
        handleSubscriptionAction("browse");
        voiceCommands.speak("Showing available channels for subscription.");
        break;
      case "unsubscribe":
        handleSubscriptionAction("view");
        voiceCommands.speak("Select a subscription to cancel.");
        break;
      case "mainMenu":
        handleBackToMenu();
        voiceCommands.speak("Returning to main menu.");
        break;
  ...
  const handleCreateChannelSubmit = async () => {
    ...
    voiceCommands.speak("Channel created successfully!");
  };
  ...
  const confirmDeleteChannel = (channelId: string) => {
    ...
    handleChannelAction("list");
    voiceCommands.speak("Channel deleted successfully!");
  };
  ...
  const confirmCancelSubscription = (subscriptionId: string) => {
    ...
    handleSubscriptionAction("view");
    voiceCommands.speak("Subscription cancelled successfully!");
  };
  const showCommands = () => {
    setMessages((prev) => [
      ...prev,
        ...
        action: (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-1 text-sm">
                <p>"Create channel" - Start channel creation</p>
                <p>"Delete channel" - Remove a channel</p>
                <p>"Edit channel" - Modify channel settings</p>
                <p>"Add package" - Add new subscription package</p>
                <p>"Subscribe to channel" - Subscribe to a channel</p>
                <p>"Show my subscriptions" - View active subscriptions</p>
                <p>"Main menu" - Return to main menu</p>
              </div>
            </div>
            <div>
              <h3 className="font-semibold text-blue-400 mb-2">
    ...
    <div className="h-full">
      <div className="relative">
        ...
        <button
          ...
          className="absolute bottom-20 right-4 p-3 rounded-full bg-[#2B5278] text-white hover:bg-[#3a6898] transition-colors"
        >
          {isListening ? (
